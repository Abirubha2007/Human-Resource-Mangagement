...
const leaveController = {
  apply: async (req, res, next) => {
    ...
    try {
      // Get employee name
      const empRes = await db.query('SELECT first_name, last_name FROM employees WHERE id = $1', [employeeId]);
  ...
  getBalance: async (req, res, next) => {
    ...
    try {
      const result = await db.query('SELECT * FROM leave_balances WHERE employee_id = $1', [employeeId]);
        ...
        if (becomingApproved) {
          ...
          await client.query(
            `UPDATE leave_balances SET
              ${typeKey}_used = ${typeKey}_used + $1,
              ${typeKey}_remaining = GREATEST(0, ${typeKey}_total - (${typeKey}_used + $1)),
              updated_at = CURRENT_TIMESTAMP
             WHERE employee_id = $2`,
