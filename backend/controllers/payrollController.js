...
const payrollController = {
  ...
  create: async (req, res, next) => {
    ...
    try {
      ...
      const empRes = await db.query('SELECT first_name, last_name, department FROM employees WHERE id = ?', [employeeId]);
      ...
      const existRes = await db.query(
        'SELECT * FROM payroll WHERE employee_id = ? AND payout_month = ?',
        [employeeId, month]
      );
  ...
  update: async (req, res, next) => {
    ...
    try {
      const checkRes = await db.query('SELECT * FROM payroll WHERE id = ?', [id]);
