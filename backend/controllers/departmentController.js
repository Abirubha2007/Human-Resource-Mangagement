...
const departmentController = {
  ...
  create: async (req, res, next) => {
    ...
    try {
      ...
      const existCheck = await db.query('SELECT * FROM departments WHERE name = ?', [name]);
  ...
  update: async (req, res, next) => {
    ...
    try {
      const checkRes = await db.query('SELECT * FROM departments WHERE id = ?', [id]);
      ...
      const countRes = await db.query('SELECT COUNT(*) FROM employees WHERE department = ?', [result.rows[0].name]);
  ...
  delete: async (req, res, next) => {
    ...
    try {
      ...
      const empCountRes = await db.query('SELECT COUNT(*) FROM employees WHERE department = ?', [dept.name]);
