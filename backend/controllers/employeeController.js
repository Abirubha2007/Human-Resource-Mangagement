...
const employeeController = {
  getAll: async (req, res, next) => {
    ...
    try {
      ...
      if (search) {
        queryText += ` AND (first_name LIKE $${paramCount} OR last_name LIKE $${paramCount} OR email LIKE $${paramCount} OR id LIKE $${paramCount})`;
        params.push(`%${search}%`);
        paramCount++;
      }
  ...
  getById: async (req, res, next) => {
    ...
    try {
      const empRes = await db.query('SELECT * FROM employees WHERE id = ?', [id]);
  ...
  create: async (req, res, next) => {
    ...
    try {
      ...
      const emailCheck = await db.query('SELECT * FROM employees WHERE email = ?', [email.toLowerCase()]);
  ...
  update: async (req, res, next) => {
    ...
    try {
      // Find employee
      const empCheck = await db.query('SELECT * FROM employees WHERE id = ?', [id]);
      ...
      if (employee.user_id) {
        await db.query(
          'UPDATE users SET email = COALESCE(?, email), role = COALESCE(?, role), updated_at = CURRENT_TIMESTAMP WHERE id = ?',
  ...
  delete: async (req, res, next) => {
    ...
    try {
      ...
      try {
        ...
        if (employee.user_id) {
          await client.query('DELETE FROM users WHERE id = ?', [employee.user_id]);
        }
  ...
  getMe: async (req, res, next) => {
    try {
      const empRes = await db.query('SELECT * FROM employees WHERE id = ?', [req.user.employeeId]);
      const employee = empRes.rows[0];
